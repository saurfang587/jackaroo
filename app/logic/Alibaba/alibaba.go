...
type Alibaba struct {
	...
	JobTypeName  string   `json:"batchName"`     //类型
	JobCategory  string   `json:"categoryName"`  //技术类型
	JobObj       string   `json:"description"`   //工作要求
	JobDetail    string   `json:"requirement"`   //工作内容 需要合并到上条中
...
func Header(cookie string) (bool, error) {
	...
	for i := 0; i < len(Allali); i++ {
		Job := &model.Job{
			ID:           strconv.Itoa(Allali[i].Id),
			Company:      "阿里巴巴",
			Title:        Allali[i].Title,
			JobCategory:  Allali[i].JobCategory,
			JobTypeName:  Allali[i].JobTypeName,
			JobDetail:    Allali[i].JobDetail + Allali[i].JobObj,
	...
	pan2, err3 := model.UpdateJobs(jobs)
	if pan2 == false {
		return false, err3
	}
	return true, nil
