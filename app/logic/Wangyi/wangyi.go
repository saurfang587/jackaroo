...
type Wangyi struct {
	...
	JobDetail         string   `json:"description"`
	JobObisity        string   `json:"requirement"`
	JobCategory       string   `json:"reqEducationName"` //
...
func Header(cookie string) (bool, error) {
	...
	for i := 0; i < len(AllWangyi); i++ {
		...
		job := &model.Job{
			ID:           strconv.Itoa(AllWangyi[i].Id),
			Company:      "网易",
			Title:        AllWangyi[i].Name,
			JobCategory:  "实习",
			JobTypeName:  AllWangyi[i].JobCategory,
			JobDetail:    AllWangyi[i].JobDetail + AllWangyi[i].JobObisity,
